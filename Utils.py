...
class RandomG:
    ...
    def update_attack(self, attack_ratio):
        BadNum = int(self.num_clients * attack_ratio)
        BIDs = list(range(BadNum))
        self.addBadCs(BIDs)
...
def load_Model(Type, Name):
    ...
    model_mapping = {
        ...
        ("alexnet", "cifar10"): (AlexNet_CIFAR10, "alexnet_cifar10"),
        ("lstm", "shakespeare"): (Char_LSTM, "lstm_shake"),
        ("fc", "mnist"): (FC_MNIST, "fc_mnist")
    }
...
def find_lambda(gradient, direction, alpha):
    start_point, end_point, iterations = 0.1, 0.001, 100
    gap = (start_point - end_point) / iterations
    
    d1, d2, d3 = dot_product(gradient, gradient), dot_product(gradient, direction), compute_norm(gradient)
    
    best_value, best_lambda = 1e6, -1
    for i in range(iterations):
        lambda_value = start_point - gap * i
        adjusted_gradient = subtract_parameters(gradient, lambda_value, direction)
        d5 = compute_norm(adjusted_gradient)
        
        value = abs(alpha - (d1 - lambda_value * d2) / d3 / d5)
        if value < best_value:
            best_value, best_lambda = value, lambda_value
